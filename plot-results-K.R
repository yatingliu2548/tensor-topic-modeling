...
directory <- "~/Documents/tensor-topic-modeling/synthetic/results_old"

# List all files in the directory
files <- list.files(path = directory, pattern = "*.csv", full.names = TRUE)
res <- c()
for (file in files[466:590]){
  ...
  temp_df["Q1"] = as.numeric(split_numbers[3])
  temp_df["Q2"] = as.numeric(split_numbers[3])
  temp_df["R"] = as.numeric(split_numbers[4])
...
res_summary <- res %>% 
  group_by(Q1, Q2, R, M, K, mode, method, sparse) %>%
