...
get_repo_root <- function() {
  ...
  if (length(file_arg) > 0) {
    cur <- dirname(normalizePath(sub("^--file=", "", file_arg[1])))
  } else {
  ...
  repeat {
    if (
      ...
      file.exists(file.path(cur, "methods", "data_generation.R")) &&
      file.exists(file.path(cur, "synthetic", "simulation.R"))
    ) {
      return(cur)
    }
    parent <- dirname(cur)
    if (identical(parent, cur)) {
      stop("Cannot find repository root from ", getwd())
    }
    cur <- parent
  }
}
...
required_packages <- c("VGAM", "rTensor", "nnTensor", "topicmodels", "stm", "Matrix", "ggplot2")
missing_packages <- required_packages[!vapply(required_packages, requireNamespace, logical(1), quietly = TRUE)]
if (length(missing_packages) > 0) {
  stop("Missing required packages: ", paste(missing_packages, collapse = ", "))
}

source(file.path(repo_root, "methods", "data_generation.R"))
source(file.path(repo_root, "methods", "analysis_function.R"))
source(file.path(repo_root, "methods", "vh_methods", "load_vh_methods.R"))
source(file.path(repo_root, "methods", "score_based_methods.R"))
...
use_saved_y3d3 <- toupper(Sys.getenv("USE_SAVED_Y3D3", unset = "FALSE")) %in% c("TRUE", "T", "1", "YES", "Y")
saved_y3_csv <- Sys.getenv("SAVED_Y3_CSV", unset = file.path(repo_root, "synthetic", "Y3.csv"))
saved_d3_csv <- Sys.getenv("SAVED_D3_CSV", unset = file.path(repo_root, "synthetic", "D3.csv"))
...
M=100
if (use_saved_y3d3) {
  message("Using saved Y3/D3 CSV files:")
  message("  Y3: ", saved_y3_csv)
  message("  D3: ", saved_d3_csv)
  Y3 <- as.matrix(read.csv(saved_y3_csv, check.names = FALSE))
  D3 <- as.matrix(read.csv(saved_d3_csv, check.names = FALSE))
  storage.mode(Y3) <- "numeric"
  storage.mode(D3) <- "numeric"
  if (nrow(Y3) != nrow(D3) || ncol(Y3) != ncol(D3)) {
    stop("Saved Y3 and D3 must have the same dimensions.")
  }
  if (ncol(Y3) != Q1 * Q2) {
    stop(sprintf("Expected %d columns from Q1*Q2, got %d.", Q1 * Q2, ncol(Y3)))
  }
  M <- as.integer(unique(colSums(Y3))[1])
  if (any(colSums(Y3) != M)) {
    stop("Saved Y3 columns must all have the same total count.")
  }
  vocab <- seq_len(nrow(Y3))
  D0 <- tensorization(D3, mode = 3, Q1, Q2, length(vocab))
  D_true <- D0@data
  tensor <- tensorization(Y3, mode = 3, Q1, Q2, length(vocab))@data
  tensor_true <- D0@data
  data <- D3
} else {
...
ggplot(errors[errors$method %in% c("NTD", "ours", "LDA"), , drop = FALSE],
