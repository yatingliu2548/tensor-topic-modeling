...
run_method <- function(data, K1, K2, K3, M, method, threshold=FALSE){
  ...
  if (method =="TTM-HOSVD"){
    elapsed_timeOurs <- system.time({
      tmp<-tryCatch(
        score(data$X, normalization="TTM", method = "HOSVD", K1=K1, K2=K2, K3=K3, M=M,
              as.sparse = FALSE),
        error = function(err) {
          # Code to handle the error (e.g., print an error message, log the error, etc.)
  ...
  if (method =="TTM-HOOI"){
    elapsed_timeOurs <- system.time({
      tmp<-tryCatch(
        score(data$X, normalization="TTM", method = "HOOI", 
  ...
  if (method =="TopicScore-HOSVD"){
    elapsed_timeOurs <- system.time({
      tmp<-tryCatch(
        score(data$X, normalization="TopicScore", method = "HOSVD", K1=K1, K2=K2, K3=K3, M=M,
              as.sparse = FALSE),
        error = function(err) {
          # Code to handle the error (e.g., print an error message, log the error, etc.)
          paste0("Error occurred while running Ours ", R, " :", conditionMessage(err), "\n")
