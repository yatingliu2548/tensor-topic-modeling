...
# Sample data preparation (modify according to your actual data)
N1 <- Q1  # Number of reviewers
N2 <- Q2  # Number of papers
V <- R  # Vocabulary size
K <- K3  # Number of topics
K1 <- K1  # Number of reviewer types
K2 <- K2  # Number of paper categories
D <- Q1 * Q2  # Total number of reviews (assuming each reviewer reviewed each paper)

# Simulate word data for each review (for simplicity)
M_list <- rep(M, D)  # Number of words in each review (constant here for simplicity)
word_ids <- array(sample(1:V, sum(W), replace = TRUE), dim = c(D, max(W)))
reviewer_ids <- rep(1:N1, each = N2)
paper_ids <- rep(1:N2, times = N1)

# Data list for Stan
data_list <- list(N1 = Q1, N2 = Q2, V = R, K = K3, K1 = K1, K2 = K2, D = D, 
                   X = t(D3), 
                  reviewer_ids = reviewer_ids, paper_ids = paper_ids)
...
fit <- sampling(stan_model, data = data_list, iter = 100, chains = 1)
