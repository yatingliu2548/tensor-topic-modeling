...
plist = c(1000,3000,10000) 
Q1list=c(Q1)
Nlist=c(100,500,1000,2500,5000,6000,7000,8000,9000,10000)#
for (k in 1:length(Q1list)){
  for (i in 1:length(Mlist)){
     for (j in 1:length(Rlist)){
        data = synthetic_dataset_creation(Q1=Q1, Q2=Q2, p=plist[j],
                                          K1=K1, K2=K2, K3=K3,
                                          alpha_dirichlet=0.1, 
                                          n_max_zipf=50000, a_zipf=1,
                                          n_anchors=2, delta_anchor=0.1, 
                                          N=Nlist[i], seed=seed, offset_zipf=2.7,
                                          vary_by_topic=TRUE, sparsity = FALSE)
